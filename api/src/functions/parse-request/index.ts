...
- urgency: one of "CRITICAL", "HIGH", "NORMAL" (or null if not clear; use context clues like "urgent", "emergency", "ASAP" for CRITICAL/HIGH)
- location: a string describing the location/hospital name (or null if not provided)
- confidence: a float between 0 and 1 indicating how confident you are in the extraction
- clarifications: an array of strings describing any ambiguities that need clarification
...
export async function parseRequest(
  ...
  try {
    ...
    const completion = await openAIClient.getChatCompletions(openAIDeployment, [

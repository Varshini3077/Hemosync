  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const parsed = await parseRequest({
        message: `Need ${units} units of ${component} blood type ${bloodType} urgency ${urgency}`,
      });
      ...
      // 2. Get ranked banks (using a default hospital location)
      const rankedResponse = await getRankedBanks({
        bloodType,
        component,
        units,
        location: { lat: 28.6139, lng: 77.209 }, // default: New Delhi coordinates
        limit: 5,
      });

      setRankedBanks(
        rankedResponse.banks.map((b) => ({
          id: b.id,
          name: b.name,
          address: b.address,
          phone: b.phone,
          distanceKm: b.distanceKm,
          estimatedDriveMinutes: b.estimatedDriveMinutes,
      ...
      // 3. Trigger broadcast
      const broadcastResponse = await triggerBroadcast({
        requestId,
        bankIds: rankedResponse.banks.map((b) => b.id),
      });

      updateBroadcastStatus({
        status: broadcastResponse.status.requestId ? "BROADCASTING" : "PENDING",
        banks: [],
      });

      // 4. Navigate to status page
      void parsed; // used for validation side-effect
